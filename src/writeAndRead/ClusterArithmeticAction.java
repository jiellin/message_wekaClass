...
public class ClusterArithmeticAction {
	...
	public ClusterArithmeticAction(String filename) {
		try {
			...
			// (1)读入样本----------------------------------------
			DataSource source = new DataSource(path + "\\AnswerData\\05"
					+ filename + ".arff");
			...
			NumCluster = Integer.parseInt(filename.substring(filename
					.indexOf("_") + 1));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public String SimpleKMeansCluster(String filename) {
		filename = "06" + filename.substring(0, filename.indexOf("_")) + "SC"
				+ filename.substring(filename.indexOf("_")) + ".txt";
		try {
			// (1)把输出写入记事本
			PrintStream ps;
			FileOutputStream fos = new FileOutputStream(path + "\\AnswerData\\"
					+ filename);
			...
			for (int i = 0; i < insdataClusterer.numInstances(); i++)
				System.out.println(KM.clusterInstance(insdataClusterer
						.instance(i)));
			// 结束输出流-----
			bos.close();
			fos.close();
	...
	public String EMCluster(String filename) {
		filename = "06" + filename.substring(0, filename.indexOf("_")) + "EM"
				+ filename.substring(filename.indexOf("_")) + ".txt";
		try {
			// (1)把输出写入记事本
			PrintStream ps;
			...
			System.setOut(ps);
			// (2)参数设置----------------------------------------
			String[] options = weka.core.Utils.splitOptions("-I 100 -N "
					+ NumCluster + " -M 1.0E-6 -S 100");
			// 学习一个EMcluster，比如EM
			...
			// 输出评价结果
			for (int i = 0; i < insdataClusterer.numInstances(); i++)
				System.out.println(EMcluster.clusterInstance(insdataClusterer
						.instance(i)));
			// 结束输出流-----
			bos.close();
			fos.close();
	...
	public String FarthestFirstCluster(String filename) {
		filename = "06" + filename.substring(0, filename.indexOf("_")) + "FF"
				+ filename.substring(filename.indexOf("_")) + ".txt";
		try {
			// (1)把输出写入记事本
			PrintStream ps;
			...
			FarthestFirst farthestFirstcluster = new FarthestFirst(); // new
																		// instance
																		// of
																		// EMcluster
			farthestFirstcluster.setOptions(options); // set the options
			farthestFirstcluster.setNumClusters(NumCluster);
			farthestFirstcluster.buildClusterer(insdataClusterer); // build the
																	// EMcluster
			// 输出评价结果
			for (int i = 0; i < insdataClusterer.numInstances(); i++)
				System.out.println(farthestFirstcluster
						.clusterInstance(insdataClusterer.instance(i)));
			// 结束输出流-----
			bos.close();
			fos.close();
			ps.close();
	...
	public String HierarchicalCluster(String filename) {
		filename = "06" + filename.substring(0, filename.indexOf("_")) + "HC"
				+ filename.substring(filename.indexOf("_")) + ".txt";
		try {
			// (1)把输出写入记事本
			PrintStream ps;
			...
			HierarchicalClusterer Hicluster = new HierarchicalClusterer();// new
																			// instance
																			// of
																			// EMcluster
			...
			// 输出评价结果
			for (int i = 0; i < insdataClusterer.numInstances(); i++)
				System.out.println(Hicluster.clusterInstance(insdataClusterer
						.instance(i)));
			// 结束输出流-----
			bos.close();
			fos.close();
